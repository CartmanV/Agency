...
def build_support_econ():
    ...
    # --- одно агентство — одно имя на странице -----------------------------
    # Каноны в agencies.yml — это подписи кнопок бота: «ИНТЕРСИТИ СЕРВИС»,
    # «ГСП», «РейнаТур». Страница же берёт имена из свода: «KMP Group»,
    # «ГСП-Сервис», «Рейна-Тур». В итоге блок «Кто зависит от поставщика»
    # (считается по выгрузке, ключ — канон) и таблица экономики (свод) звали
    # одно агентство по-разному, и читатель видел разные компании.
    # Показываем везде написание свода — им пользуются в работе; канон
    # остаётся ключом сшивки и не переименовывается, иначе поедет бот.
    # Ушедшие агентства (Да-Тревел, ATPI, Демлинк, FS Travel) в своде
    # отсутствуют — для них канон и есть единственное имя.
    display_of = {a["canon"]: a["name"] for a in agencies if a.get("canon")}
    for row in (supplier or {}).get("byAgency", []):
        row["name"] = display_of.get(row["canon"], row["canon"])
